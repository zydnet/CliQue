...
export default function DealDetailPage() {
  ...
  useEffect(() => {
    if (params.id) {
      ...
      const foundDeal = allDeals.find((d) => d.id === id);
  ...
  const handleJoinDeal = async () => {
    if (!authManager.isAuthenticated()) {
      router.push(
        `/auth?redirectTo=${encodeURIComponent(`/deals/${params.id}`)}`
      );
      return;
    }
  ...
  const progressPercentage = Math.min(
    (deal.currentCount / deal.targetCount) * 100,
    100
  );
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div className="space-y-4">
            <div className="relative">
              <Image
                src={
                  deal.product?.imageUrls[selectedImage] ||
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                <Badge className="bg-accent-500 text-white font-semibold">
                  Save {percentage}%
                </Badge>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    {deal.product?.vendor?.description}
                  </p>
                </div>
                <div className="flex items-center gap-4 text-sm">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-gray-700 dark:text-gray-300">
                      {deal.product?.vendor?.rating} rating
                    </span>
                  </div>
          ...
          <Card className="w-full max-w-md bg-white dark:bg-gray-900">
            ...
            <CardContent className="space-y-4">
              <p className="text-gray-600 dark:text-gray-400">
                You've successfully added this item to your cart! You can
